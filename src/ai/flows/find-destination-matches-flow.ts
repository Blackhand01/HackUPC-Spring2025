...
'use server';
/**
 * @fileOverview An AI agent that finds and ranks travel destinations.
 * It uses Gemini for semantic matching based on user preferences,
...
const FindDestinationMatchesInputSchema = z.object({
  durationDays: z.number().optional().describe("Approximate duration of the trip in days."),
...
const destinationRankingPrompt = ai.definePrompt({
  ...
  output: {
    schema: z.object({
       // Expecting an array of objects with destination and score
       rankedDestinations: z.array(
            z.object({
                destination: z.string().describe("The IATA code of the destination."),
                score: z.number().min(0).max(1).describe("The affinity score (0-1)."),
            })
       ).describe("Array of destinations ranked by thematic fit."),
    }),
  },
...
Return JSON array with scores (0 to 1) for each candidate destination provided, like [{"destination": "BCN", "score": 0.92}, ...]. Only include destinations from the provided list.`;
        ...
        if (!output || !output.rankedDestinations) {
            ...
            candidateDestinationIatas.forEach(iata => affinityScores[iata] = 0.5); // Default score
             ...
             console.log("Received from Gemini:", output.rankedDestinations);
             output.rankedDestinations.forEach(item => {
                 // Ensure only candidate destinations are included
                 if (candidateDestinationIatas.includes(item.destination)) {
                     affinityScores[item.destination] = item.score;
                 }
             });
    ...
    console.log("Affinity Scores:", affinityScores);
    ...
    const enrichedDestinationsPromises = candidateDestinationIatas.map(async (destIata): Promise<EnrichedDestination> => {
        ...
        try {
            flightData = await getFlightIndicativeData({
             ...
             console.log(`Skyscanner data for ${destIata}:`, flightData);
        } catch (error) {
    ...
    const rankedDestinations = enrichedDestinations.sort((a, b) => (b.finalScore ?? 0) - (a.finalScore ?? 0));
...
// Note: The Skyscanner API Key should be securely managed, ideally via environment variables.
// The getFlightIndicativeData function needs to be implemented in '@/services/skyscannerAPI'.
